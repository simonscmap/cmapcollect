...
def load_options():
    ...
    parser.add_option("--user", "-u", help="the user name (string)")

    parser.add_option("--pwd", "-p", help="the user password (string)")

    parser.add_option(
        "--auth-mode",
        default=motu_api.AUTHENTICATION_MODE_CAS,
        help="the authentication mode: '"
        + motu_api.AUTHENTICATION_MODE_NONE
        + "' (for no authentication), '"
        + motu_api.AUTHENTICATION_MODE_BASIC
        + "' (for basic authentication), or '"
        + motu_api.AUTHENTICATION_MODE_CAS
        + "' (for Central Authentication Service) [default: %default]",
    )

    parser.add_option("--proxy-server", help="the proxy server (url)")

    parser.add_option("--proxy-user", help="the proxy user (string)")

    parser.add_option("--proxy-pwd", help="the proxy password (string)")

    parser.add_option("--motu", "-m", help="the motu server to use (url)")

    parser.add_option("--service-id", "-s", help="The service identifier (string)")

    parser.add_option(
        "--product-id", "-d", help="The product (data set) to download (string)"
    )
    ...
    parser.add_option(
        "--out-name", "-f", help="The output file name (string)", default="data.nc"
    )
...
def check_version():
    ...
    if (
        cur_version[0] > REQUIRED_VERSION[0]
        or cur_version[0] == REQUIRED_VERSION[0]
        and cur_version[1] >= REQUIRED_VERSION[1]
    ):
        return
    else:
        raise Exception(
            "This tool uses python 2.7 or greater. You version is %s. "
            % str(cur_version)
        )
