...
class StopWatch(object):
    ...
    def elapsed(self, label=GLOBAL):
        ...
        """
        t0 = self.times[label] if label in self.times else 0.0
        t1 = self.timers[label] if label in self.timers else 0.0
        t2 = self.__time() if label in self.timers else 0.0
    ...
    def __str__(self):
        ...
        """
        keys = self.times.keys() + filter(
            lambda x: x not in self.times.keys(), self.timers.keys()
        )
        txt = ""
        for key in keys:
            txt = (
                ...
                + key
                + " : "
                + str(self.elapsed(key))
                + " s "
                + ("(running)" if self.isRunning(key) else "(stopped)")
                + "\n"
            )
        return txt
