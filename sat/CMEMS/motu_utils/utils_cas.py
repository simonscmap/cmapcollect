...
def authenticate_CAS_for_URL(url, user, pwd, **url_config):
    ...
    if connexion.url == url:
        raise Exception(
            utils_messages.get_external_messages()[
                "motuclient.exception.authentication.not-redirected"
            ]
            % server
        )
    ...
    opts = utils_http.encode(
        utils_collection.ListMultimap(username=quote(user), password=quote(pwd))
    )
    ...
    try:
        ...
        if e.code == 400:
            log.error(
                """Error: Bad user login or password:
