...
def build_params(_options):
    ...
    if _options.describe:
        ...
        query_options.insert(
            action="getSize", service=_options.service_id, product=_options.product_id
        )
    else:
    ...
    if _options.extraction_vertical:
        query_options.insert(z_lo=_options.depth_min, z_hi=_options.depth_max)
...
def check_options(_options):
    ...
    if (
        ...
        raise Exception(
            utils_messages.get_external_messages()[
                "motuclient.exception.option.invalid"
            ]
            % (
                _options.auth_mode,
                "auth-mode",
                [
                    ...
                    AUTHENTICATION_MODE_BASIC,
                    AUTHENTICATION_MODE_CAS,
                ],
            )
        )

    # if authentication mode is set we check both user & password presence
    if _options.user == None and _options.auth_mode != AUTHENTICATION_MODE_NONE:
        raise Exception(
            utils_messages.get_external_messages()[
                "motuclient.exception.option.mandatory.user"
            ]
    ...
    if _options.pwd == None and _options.user != None:
        raise Exception(
            utils_messages.get_external_messages()[
                "motuclient.exception.option.mandatory.password"
            ]
            % ("pwd", _options.user)
        )
    ...
    if _options.user != None and _options.auth_mode == AUTHENTICATION_MODE_NONE:
        raise Exception(
            utils_messages.get_external_messages()[
                "motuclient.exception.option.mandatory.mode"
            ]
    ...
    if _options.motu == None:
        raise Exception(
            utils_messages.get_external_messages()[
                "motuclient.exception.option.mandatory"
            ]
            % "motu"
        )
    ...
    if not out_dir.startswith("console"):
        ...
        if not os.path.exists(out_dir):
            raise Exception(
                utils_messages.get_external_messages()[
                    "motuclient.exception.option.outdir-notexist"
                ]
                % out_dir
            )
        ...
        if not os.access(out_dir, os.W_OK):
            raise Exception(
                utils_messages.get_external_messages()[
                    "motuclient.exception.option.outdir-notwritable"
                ]
                % out_dir
            )
    ...
    if (_options.proxy_server != None) and (len(_options.proxy_server) != 0):
        ...
        if not m:
            raise Exception(
                utils_messages.get_external_messages()[
                    "motuclient.exception.option.not-url"
                ]
                % ("proxy-server", url)
            )
        ...
        if (_options.proxy_user != None) != (_options.proxy_pwd != None):
            raise Exception(
                utils_messages.get_external_messages()[
                    "motuclient.exception.option.linked"
                ]
    ...
    if (
        _options.latitude_min != None
        or _options.latitude_max != None
        or _options.longitude_min != None
        or _options.longitude_max != None
    ):
        _options.extraction_geographic = True
        if _options.latitude_min == None:
            raise Exception(
                utils_messages.get_external_messages()[
                    "motuclient.exception.option.geographic-box"
                ]
                % "latitude_min"
            )
        ...
        if tempvalue < -90 or tempvalue > 90:
            raise Exception(
                utils_messages.get_external_messages()[
                    "motuclient.exception.option.out-of-range"
                ]
...
def get_url_config(_options, data=None):
    ...
    if _options.proxy:
        ...
        kargs["proxy"] = {"scheme": proxyUrl.scheme, "netloc": proxyUrl.netloc}
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                and len(headers["Content-Type"]) > 0
                and isADownloadRequest
                and (
                ...
                raise Exception(
                    utils_messages.get_external_messages()[
                        "motuclient.exception.motu.error"
                    ]
                    % m.read()
                )
            ...
            if "Content-Length" in headers:
                try:
                    ...
                    log.info(
                        "File size: %s (%i B)" % (utils_unit.convert_bytes(size), size)
                    )
                except Exception as e:
                    size = -1
                    log.warn(
                        "File size is not an integer: %s" % headers["Content-Length"]
                    )
            ...
            def none_function(sizeRead):
                ...
                percent = 100
                log.info(
                    "- %s (%.1f%%)", utils_unit.convert_bytes(size).rjust(8), percent
                )
                lastProgressPercentValue = percent
    ...
    try:
        ...
        for option in dir(_options):
            if not option.startswith("_"):
                log.log(
                    utils_log.TRACE_LEVEL, "%s=%s" % (option, getattr(_options, option))
                )
        ...
        try:
            ...
            if (
                _options.sync == True
                or _options.describe == True
                or _options.size == True
            ):
                    ...
                    while True:
                        if _options.auth_mode == AUTHENTICATION_MODE_CAS:
                            ...
                            requestUrlCas = utils_cas.authenticate_CAS_for_URL(
                                requestUrl, _options.user, _options.pwd, **url_config
                            )
                            stopWatch.stop("authentication")
                        else:
                            # if none, we do nothing more, in basic, we let the url requester doing the job
